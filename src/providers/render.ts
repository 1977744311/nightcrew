...
function header(spec: WorkSpec): string[] {
  return [
    ...
    "confirmation. If something is genuinely undecidable, append it to",
    `\`${NIGHTCREW_DIR}/questions.md\` and move on within your scope, formatted so the`,
    "operator can answer with one click:",
    "",
    "```md",
    "- [ ] (<iso-minute>) <one-line question>",
    "      - A: <resolution> (recommended)",
    "      - B: <alternative> => backlog: <one-line work item>",
    "```",
    "",
    "Offer 2-4 lettered options, mark exactly one `(recommended)`, and suffix",
    "`=> backlog: ...` to any option whose adoption needs new code work.",
    "",
  ];
}
...
function planOpTask(): string[] {
  ...
  return [
    ...
    `created: ${today}`,
    "parallel: false",
    "backlog: <exact unchecked BACKLOG first-line text, optional>",
    "---",
    "",
    "## Goal",
    ...
    "- If you choose one unchecked BACKLOG item, set `backlog` to that item's exact first-line text after `- [ ] `.",
...
function gardenOpTask(): string[] {
  return [
    ...
    "",
    "1. In `questions.md`, fold answered entries (checked, or carrying an `answer:` line) into a one-line decision note; drop stale ones.",
    "2. In `questions.md`, redraft open entries that carry a `feedback:` line: replace their lettered options with 2-4 new ones that address the feedback (one `(recommended)`, `=> backlog: ...` where adoption needs code work), remove the `feedback:` line, keep the entry unchecked. Give the same option treatment to open entries that have no options yet.",
    "3. In `qa.md`, drop entries that are demonstrably fixed or already covered by a BACKLOG item; keep the file lean.",
    "4. Keep every file small and pointer-like: prune duplicated prose, dead references, stale plan mentions.",
    "",
    "`crew.md` belongs to the operator — never edit it. If BACKLOG items look",
    `done (their plans sit in \`plans/completed/\`), say so in your final message`,
    "so the operator can tick them.",
    "",
