...
## Acceptance
- [ ] `nightcrew propose refine [<id-or-file>] --feedback "<text>"` defaults to the latest pending proposal, reruns all three proposal lenses with the original goal, previous candidate summaries, and feedback in each prompt, writes a new pending artifact with optional `refinedFrom` and feedback fields, and archives the source artifact.
- [ ] In TTY picker flows, zero selections prompt once for optional feedback; empty feedback leaves the proposal pending and unchanged, while non-empty feedback refines and reopens the picker on the new artifact.
- [ ] Fake-provider tests cover feedback in rerun prompts, lineage fields, source archival, and zero-selection/no-feedback no-op behavior, with `CHANGELOG.md` updated under `## Unreleased`.
