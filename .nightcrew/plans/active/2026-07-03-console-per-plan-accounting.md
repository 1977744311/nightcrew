...
## Acceptance
- [x] A shared aggregation module returns plan id, title, iterations, total tokens, durationMs, and landed/pending status, and `nightcrew report` uses it without changing the existing summary contract.
- [x] `src/console/data.ts` exposes stable per-plan metrics in project detail JSON, and the project detail HTML renders a per-plan table with token totals and human-readable duration while preserving existing summary, proposals, token curve, and history table.
- [x] Tests cover the aggregation module, console detail JSON shape, and the HTML rendering path.
