...
## Acceptance
- [ ] `schema/config.schema.json` is generated from the zod config schema with zod v4 `z.toJSONSchema`, committed, and reproducible through an `npm run schema` script without new dependencies.
- [ ] A focused test regenerates the schema and fails when the committed schema is out of sync.
- [ ] `nightcrew init` emits a config template beginning with a `# yaml-language-server: $schema=` comment pointing at the raw GitHub URL for the committed schema.
- [ ] `CHANGELOG.md` records the landed change under `## Unreleased`.
